...
def get_platform():
    ...
    platform: str
    while True:
        print(f"What platform are you on? {Fore.YELLOW}PC{Style.RESET_ALL}, {Fore.GREEN}XBOX"
              f"{Style.RESET_ALL}, {Fore.BLUE}PS4{Style.RESET_ALL}, or {Fore.RED}Switch"
              f"{Style.RESET_ALL}?")
...
def logic():
    ...
    """
    get_platform()
    print('What part would you like to find the price of? Please use the form of {Fore.CYAN}"Braton'
          f'Prime Set"{Style.RESET_ALL}: ')

    # get user input and sends it to lowercase, then stripstrailing spaces
    ...
    if net_error_checking(page.status_code):
        ...
        try:
            print(f"The going rate for a {Fore.CYAN}{name_of_item}{Style.RESET_ALL} is {Fore.CYAN}"
                  f"{find_avg(item_list, now)}{Style.RESET_ALL}.")
