###########
 Licensing
###########

******************
 Acknowledgements
******************

|  This project uses the tools listed in `prek.toml
   <https://github.com/Eutropios/WarMAC/blob/main/prek.toml>`_ and
   `pyproject.toml
   <https://github.com/Eutropios/WarMAC/blob/main/pyproject.toml>`_ in its
   development.

***********
 Licensing
***********

|  **This project is NOT affiliated with Warframe, Digital Extremes, or Warframe
   Market.**

|  WarMAC Documentation, © 2024 by Noah Jenner, is licensed under CC BY-SA 4.0.
   To view a copy of this license, visit `<https://creativecommons.org/licenses/by-sa/4.0/>`_.

|  WarMAC, (C) 2023-2026 by Noah Jenner, is licensed under GNU GPL version
   3.0-or-later. Please see `LICENSE.txt
   <https://github.com/Eutropios/WarMAC/blob/main/LICENSE.txt>`_ for the full
   license information.

|  For licensing regarding dependencies, please see the `LICENSES
   <https://github.com/Eutropios/WarMAC/blob/main/LICENSES>`_ directory.
